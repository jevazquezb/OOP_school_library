...
p Rental.new('2022-06-25', lab, miguel)
p Rental.new('2022-08-13', lab, julio)

p miguel.rentals.count
p miguel.rentals.map(&:date)
p termo.rentals.count
p lab.rentals.count
p(lab.rentals.map { |rental| rental.book.author })
p(lab.rentals.map { |rental| rental.person.name })

p Rental.new('2022-07-18', termo, julio)
p Rental.new('2022-08-04', lab, manu)

p julio.rentals.count
p julio.rentals.map(&:date)
p termo.rentals.count
